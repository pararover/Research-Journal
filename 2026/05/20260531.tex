...
ixpe*_det?_evt2_v01.fits
		...
		\begin{enumerate}
			...
			\item Make use of the steps mentioned in the journal entry dated 23 Sep. 2025 (Tuesday).
		...
		\begin{enumerate}
			...
			\item Locate the row containing the ObsID of interest.
			
			\item Copy the entry under the time column for that row.
...
quzcif IXPE GPD DU1 - MATRIX 2023-10-17 16:41:45.850 -
			...
			\item Download the \verb|alpha075| file using the \verb|wget| command as follows:
			\begin{verbatim}
wget -v <<FTP URL>>
			\end{verbatim}
			For example:
...
ls det1_att
			...
			\item Then run the \verb|ixpecalcarf| as per steps mentioned in journal entry dated 16 Oct. 2025 (Thursday).
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\item Run the \verb|fthedit| as per steps mentioned in journal entry dated 16 Oct. 2025 (Thursday).
		\end{enumerate}
	\end{itemize}